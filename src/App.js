...
function App() {
  ...
  const getProfiles = async () => {
    if(success_message == 'succeeded')
    {
      ...
      try {
        const fetchData  = await axios.post('https://cors-chi.vercel.app/cors-post', { json },
        {
          headers: {
  ...
  const getProfiles2 = async () => {
      try {
        ...
        if(JSON.stringify(successFirstAPI.success)) {
          ...
          const fetchEmail  = await axios.post('https://cors-chi.vercel.app/cors-post-email', { mergedObj },
          {
            headers: {
...
const handleClick = () => {
  ...
  fetch("https://cors-dnv6gur6l-faizanshahid232.vercel.app/create-payment-intent", {
    method: "POST",
    headers: { 
      "Content-Type": "application/json",
