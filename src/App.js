...
function App() {
  ...
  const getProfiles = async () => {
    if(success_message == 'succeeded')
    {
      ...
      try {
        const fetchData  = await axios.post('https://payment.julibs.com/cors-post', { json },
        {
          headers: {
  ...
  const getProfiles2 = async () => {
      try {
        ...
        if(JSON.stringify(successFirstAPI.success)) {
          ...
          const fetchEmail  = await axios.post('https://payment.julibs.com/cors-post-email', { mergedObj },
          {
            headers: {
